...
export const Popover = ({ popover, onClose, onPopoverClose }) => {
  return (
    <>
      {popover && (
        <PopoverBox onClose={onClose} onPopoverClose={onPopoverClose} />
      )}
    </>
  );
};

const PopoverBox = ({ onClose, onPopoverClose }) => {
  const handleCross = () => {
    onPopoverClose();
    onClose();
  };
  ...
  return (
    <div className="w-[375px] absolute m-0 left-[50%] top-[40%]  text-sm z-[8]">
      <div
        className="relative left-[-50%] top-[-50%] rounded-lg border-[1px]
    border-[#d5d9d9] bg-white shadow-[0_0_14px_0_rgba(15,17,17,.5)] h-full overflow-hidden"
      >
        <header className="border-b-[1px] border-[#d5d9d9] text-left    bg-[#f0f2f2] relative    rounded-t-lg  py-0 px-6 flex justify-between font-bold">
          ...
          <IoIosClose className=" scale-[2.5] mt-5" onClick={handleCross}/>
        </header>
